...
function Check() {
  return (
    <svg className="w-3.5 h-3.5 shrink-0 text-emerald-500" viewBox="0 0 14 14" fill="none" aria-hidden="true">
      <path d="M2 7L5.5 10.5L12 3.5" stroke="currentColor" strokeWidth="1.8"
    ...
    <section className="relative min-h-[calc(100vh-64px)] flex items-center overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
        ...
        <div
          ...
          style={{
            background: "linear-gradient(160deg, #eff6ff 0%, #ffffff 45%, #faf5ff 100%)",
          }}
        />
        {/* Subtle dot grid */}
        <div
        ...
        <div
          className="absolute bottom-0 -left-20 w-[500px] h-[400px]"
          style={{
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 py-16 lg:py-20">
        <div className="grid lg:grid-cols-[1fr_1fr] gap-10 xl:gap-16 items-center">
          ...
          >
            ...
            <motion.h1
              variants={fadeUp}
              className="text-[2.6rem] sm:text-5xl xl:text-[3.5rem] font-bold leading-[1.08] tracking-tight"
            >
              {hero.headlineParts.map((part, i) =>
                part.gradient ? (
                  <span
                    key={i}
                    className="block bg-gradient-to-r from-blue-600 via-violet-500 to-purple-600 bg-clip-text text-transparent"
                  >
                    {part.text}
                  </span>
                ) : (
              ...
              <button
                ...
                onMouseEnter={(e) => {
                  (e.currentTarget as HTMLButtonElement).style.boxShadow =
                    "0 8px 24px rgba(37, 99, 235, 0.5), inset 0 1px 0 rgba(255,255,255,0.15)";
                  (e.currentTarget as HTMLButtonElement).style.transform = "translateY(-1px)";
                }}
                onMouseLeave={(e) => {
                  (e.currentTarget as HTMLButtonElement).style.boxShadow =
                    "0 4px 16px rgba(37, 99, 235, 0.35), inset 0 1px 0 rgba(255,255,255,0.15)";
            ...
            <motion.div variants={fadeUp} className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <p className="text-sm text-slate-500">
                  <span className="font-semibold text-slate-700">120+ traders</span>{" "}
                ...
                {!submitted ? (
                  ...
                  >
                    <input
                      ...
                      required
                      className="flex-1 text-sm text-slate-900 placeholder:text-slate-400 rounded-xl px-4 py-3 outline-none transition-all duration-150 bg-white border border-slate-200"
                      onFocus={(e) => {
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className="shrink-0 inline-flex items-center justify-center gap-2 px-5 py-3 text-sm font-semibold text-white rounded-xl transition-all duration-150 disabled:opacity-60 disabled:cursor-not-allowed"
                      style={{
                  ...
                  >
                    <div className="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center shrink-0">
                      <svg className="w-4 h-4 text-emerald-600" viewBox="0 0 16 16" fill="none" aria-hidden="true">
                        <path d="M3 8L6.5 11.5L13 4.5" stroke="currentColor" strokeWidth="2"
                          strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    </div>
          ...
          <motion.div
            ...
            transition={{ duration: 0.9, delay: 0.25, ease: [0.16, 1, 0.3, 1] as [number, number, number, number] }}
            className="hidden lg:flex items-center justify-center lg:justify-end xl:justify-center"
          >
            <DashboardMockup />
          </motion.div>
        </div>
